      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mt-4">
          <div className="flex justify-between items-center mb-4">
            <div className="text-pink-500 font-bold text-2xl">{panel.price ? `${panel.price} USD` : ''}</div>
          ...
          <div className="mb-4">
            ...
            <ul className="list-disc pl-6 text-gray-700 space-y-2">
              {panel.testTypesNames && panel.testTypesNames.map((name: string, idx: number) => (
                <li key={name}><b>{name}:</b> {panel.testTypesDescriptions && panel.testTypesDescriptions[idx]}</li>
              ))}
            </ul>
          </div>
