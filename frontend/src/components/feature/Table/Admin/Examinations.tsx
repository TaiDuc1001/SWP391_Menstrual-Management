...
const Examinations: React.FC<ApproveResultTableProps> = ({
    ...
    const getStatusBadge = (status: string) => {
        ...
        switch (status) {
            case 'EXAMINED':
                return <span
                    className="bg-blue-100 text-blue-600 px-3 py-1 rounded-full text-xs font-medium">{label}</span>;
            case 'COMPLETED':
                return <span
                    className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-xs font-medium">{label}</span>;
            case 'IN_PROGRESS':
                return <span
                    className="bg-yellow-100 text-yellow-600 px-3 py-1 rounded-full text-xs font-medium">{label}</span>;
            case 'CANCELLED':
                return <span
                    className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-xs font-medium">{label}</span>;
            default:
                return <span
                    className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-medium">{label}</span>;
        }
    };
