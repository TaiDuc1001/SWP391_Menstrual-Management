        ...
        {options.map((option) => (
          <li key={option.label} className="">
            ...
            >
              {({ isActive }) => (
                <>
                  <span className={`absolute left-0 top-2 bottom-2 w-1 rounded-full transition-all duration-200 ${isActive ? 'bg-pink-400' : 'bg-transparent group-hover:bg-pink-200'}`}></span>
